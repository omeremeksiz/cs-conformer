...
file_path = "data/VRSA-FR/SSQ_discrete.xlsx"
...
selected_values = pd.DataFrame([[0]*4]*25)
for subject in df.T:
    for scene_index in range(0, 20, 2):
        ...
        if (selected_values == scene[0]).sum().sum()>7 or (selected_values == scene[1]).sum().sum()>7:
            continue
...
for subject in selected_indices.T:
    for index in selected_indices.iloc[subject]:
